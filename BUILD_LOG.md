...
### 2026-08-24 — Grok meter blanked on 6h OIDC expiry
The Grok card went plan-only again overnight. Live on this machine: `cli-chat-proxy` credits still 200 (`creditUsagePercent: 21`), but Grok OIDC access tokens last **6 hours** (`iat` 13:33 UTC → `exp` 19:33 UTC on a fresh `grok` launch). NMT never refreshed; `GrokCredentialLoader` threw `.expired`; `staleFallback` returned `success(planName, windows: [])`; `ProviderCardView` hides "No usage windows reported" for Grok. Net: a green live card with no weekly bar until `grok` happened to run and rewrite `auth.json`. Opening this session refreshed the token at 09:33 and NMT's 120s poll at 09:34 showed 21% — which is why the snapshot looked fine while the morning card did not.
Fix: (1) cache last windows + resetCount; expiry/transport failure keeps the bar (plan-name-only cache is NOT a fallback — that was the silent bug). (2) refresh at `POST https://auth.x.ai/oauth2/token` (public PKCE client, no secret) and write the new access token (and a rotated refresh token, if any) back to `~/.grok/auth.json` so grok CLI's sibling-refresh path can adopt it. 401 on credits maps to the expired re-auth message. `swift test` 172 passed.

### 2026-08-23 — Grok reset bank
SuperGrok banked usage resets live on grok.com Settings ▸ Usage ("Reset Available"). Pollable count: `POST https://grok.com/prod_mc_billing.ConsumerUiSvc/GetRemainingResets` (gRPC-web, empty protobuf, same OIDC bearer as credits). Response `ConsumerGetRemainingResetsResp.tokens[]` of `ConsumerResetToken { token_id=10, validity_end=30 }`. Count unexpired tokens only. Live on this machine: 1 token, `validity_end` 2026-09-12 18:49 UTC. Do not POST `RedeemReset` (spends the token). Card shows "N resets banked" + "Use a reset…" → `https://grok.com/?_s=usage`. Remaining-resets failure leaves `resetCount` nil and keeps the weekly bar. `swift test` 164 passed.
